    ...
    char **                             argv)
{
    ...
    result = globus_xio_driver_load("bidi", &driver);
    test_result(result);
    result = globus_xio_stack_push_driver(source_stack, driver);
    test_result(result);
    globus_list_insert(&driver_list, driver);
    test_result(globus_xio_attr_init(&attr));  
    ...
    for(i = 1; i < argc; i++)
    {
        ...
        {
	    globus_size_t	portnumber;
            portnumber = atoi(argv[++i]);
            test_result(globus_xio_attr_cntl(
                attr,
                driver,
    ...
    if(source_is_server)
    {
         ...
         * then open it
         */
        result = globus_xio_server_create(&server, attr, source_stack);
        test_result(result);
